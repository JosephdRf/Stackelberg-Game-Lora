...
#SBATCH --job-name=Exp2_2

# Exp2_2 (Qwen) — confidence loss "max" sur les têtes leader.
#   L_leader   = CE + λ_conf·(-1/L Σ_l max_l' A_leader[l,l'])     λ_conf = 1.0
#   L_follower = CE + λ_lead·cos(A_i,A_leaders) + λ_peer·cos(A_i,A_j)   (λ=1e-2)
# Maximise la concentration de l'attention des têtes leader.
LR_LEADER=3e-4
LR_FOLLOWER=3e-4
LR_SIM=1e-4
