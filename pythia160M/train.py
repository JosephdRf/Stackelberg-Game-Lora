"""
Module commun pour l'entraînement LoRA de Pythia-160M sur The Pile.
Partagé entre le baseline et les expériences GAME-LoRA / Stackelberg.

Architecture Pythia (GPT-NeoX) :
  - 12 couches, 12 têtes, hidden_size=768, context=2048
  - Projections QKV fusionnées : "query_key_value"  (vs q/k/v séparés chez Qwen)
  - Projection output          : "dense"             (vs o_proj chez Qwen)
  - Tokenizer GPT-NeoX (vocab 50 304, vs 151 936 chez Qwen)
  - dtype de référence : float16 (vs bfloat16 chez Qwen)
...
class TrainConfig:
    ...
    # LoRA
    # Pythia (GPT-NeoX) fusionne Q/K/V dans un seul module "query_key_value"
    # + "dense" pour la projection de sortie (équivalent de o_proj chez Qwen)
    lora_rank: int = 16
    lora_alpha: int = 32   # scaling = alpha/rank = 2.0
    ...
    # Optimisation
    # Pythia a été pré-entraîné avec lr=6e-4 ; pour le fine-tuning LoRA on reste à 3e-4.
    # Pythia-160M tient facilement en mémoire avec batch_size=8 sur A100-40G.
    lr: float = 3e-4
    weight_decay: float = 0.1
    warmup_ratio: float = 0.02    # 2% warmup (même protocole que l'article)
    batch_size_per_gpu: int = 8   # Pythia-160M (~162M params) ≈ 3× moins lourd que Qwen-0.5B
    grad_accum: int = 2           # batch effectif = 16 (identique à Qwen)
...
def log_config(cfg: TrainConfig):
    logger.info("=== Configuration (Pythia-160M) ===")
