"""
Baseline : LoRA fine-tuning de Qwen2.5-0.5B sur WikiText-103 (CE seul, sans losses GAME).

Port du baseline Pythia (pythia160M/baseline/train_baseline.py) adapté à Qwen.
Mêmes hyperparamètres LoRA que exp1 (r=32, alpha=64, dropout=0.1,
target_modules=[q_proj, k_proj, v_proj, o_proj]) — seule la loss diffère.

Usage :
    python qwen2.5_0.5B/baseline/train_baseline.py
    python qwen2.5_0.5B/baseline/train_baseline.py --dry_run
    python qwen2.5_0.5B/baseline/train_baseline.py --wandb_project my_proj --run_name qwen_lora_baseline_v1
"""
...
# Repo root
_HERE    = os.path.dirname(os.path.abspath(__file__))          # qwen2.5_0.5B/baseline/
...
def train(cfg: TrainConfig, head_log_layer: int = 19,
    ...
    pbar = tqdm(total=total_steps, desc=f"Training (Qwen2.5-0.5B {_mode} baseline)", unit="step",
                disable=not sys.stderr.isatty())

    _autocast_dtype = torch.bfloat16
    _use_autocast   = device.type in ("cuda", "cpu")
    ...
    ax1.set_title(f"Training — Qwen2.5-0.5B {_mode} baseline on WikiText-103")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Baseline LoRA fine-tuning — Qwen2.5-0.5B sur WikiText-103 (CE seul)"
    )
    parser = add_common_args(parser)
    parser.add_argument("--output_dir",      default=os.path.join(_HERE, "checkpoints"))
    parser.add_argument("--run_name",        default="baseline_lora_qwen")
