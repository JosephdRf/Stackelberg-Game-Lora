"""
Module commun pour le LoRA fine-tuning de Qwen2.5-0.5B sur WikiText-103.
Port du module Pythia (pythia160M/train_utils.py), adapté à l'architecture
Qwen2 (projections q_proj/k_proj/v_proj/o_proj séparées + GQA 14 Q / 2 KV).

Partagé entre le baseline LoRA et exp1 (Stackelberg) au niveau racine
qwen2.5_0.5B/. game_lora/ continue à utiliser train.py (intact).

Contient :
  - TrainConfig (defaults Qwen)
  - WikiTextDataset (map-style, packing)
  - build_model_and_tokenizer (LoRA via PEFT, target_modules q/k/v/o_proj)
  - setup_training, evaluate, seed_everything
  - log_config / add_common_args
  - HeadInteractionMatrix (math identique Pythia, 14×14)
  - HeadOutputCapture, get_output_projection_weights (chemin Qwen2)
  - log_head_matrices
"""
...
# ---------------------------------------------------------------------------
# Architecture Qwen2.5-0.5B (constantes utilisées par gradient_mask + losses)
# ---------------------------------------------------------------------------

QWEN_HIDDEN_SIZE = 896
QWEN_N_Q_HEADS = 14
QWEN_N_KV_HEADS = 2
QWEN_D_HEAD = QWEN_HIDDEN_SIZE // QWEN_N_Q_HEADS  # 64
QWEN_KV_SIZE = QWEN_N_KV_HEADS * QWEN_D_HEAD       # 128
QWEN_GROUP_SIZE = QWEN_N_Q_HEADS // QWEN_N_KV_HEADS  # 7
...
class TrainConfig:
    ...
    # Données — WikiText-103
    dataset_name: str = "Salesforce/wikitext"
    dataset_config: str = "wikitext-103-raw-v1"
    seq_len: int = 1024
    total_tokens: int = 100_000_000  # ~1 epoch sur WikiText-103

    # LoRA — rank bumpé vs Pythia (r=16) pour le scale-up
    lora_rank: int = 32
    lora_alpha: int = 64           # scaling = alpha/rank = 2
    lora_dropout: float = 0.1
    # Qwen2 : projections séparées q/k/v + sortie o_proj
    ...
    log_every: int = 20
    save_every: int = 500
    wandb_project: Optional[str] = "Stackelberg-Qwen0.5B"
...
def log_config(cfg: TrainConfig):
    logger.info("=== Configuration (Qwen2.5-0.5B, LoRA fine-tuning) ===")
    ...
    logger.info(f"  LoRA targets     : {cfg.lora_target_modules}")
...
def add_common_args(parser):
    ...
    parser.add_argument("--wandb_group",   default=None,
                        help="Groupe W&B (ex: 'baseline', 'exp1')")
    ...
    parser.add_argument("--lora_rank", type=int, default=32,
                        help="Rang LoRA (défaut 32 pour Qwen-0.5B, bump vs Pythia r=16)")
    parser.add_argument("--lora_alpha", type=int, default=None,
                        help="Alpha LoRA (défaut 2×lora_rank si non spécifié)")
    parser.add_argument("--head_log_layer", type=int, default=19,
                        help="Couche d'attention observée pour les matrices ω/ρ/G "
                             "(layer 19 ≈ 79%% du réseau Qwen-0.5B à 24 couches)")
    return parser


# ---------------------------------------------------------------------------
# Head Interaction Matrix  (Definition 2.3 — identique Pythia, math agnostique)
# ---------------------------------------------------------------------------


class HeadInteractionMatrix:
    """
    G ∈ R^{H×H} avec G_ij = ω_ij · ρ_ij. Math identique au port Pythia.
    Pour Qwen-0.5B, H = 14 (au lieu de 12).
    """

    @staticmethod
    def compute_weight_coupling(W_O: torch.Tensor) -> torch.Tensor:
        """Args: W_O (H, d, d_h). Returns omega (H, H)."""
        W_flat = W_O.reshape(W_O.shape[0], -1).float()
        norms = W_flat.norm(dim=1, keepdim=True).clamp(min=1e-8)
        W_normed = W_flat / norms
        omega = W_normed @ W_normed.T
        return omega

    @staticmethod
    def _eta_stats(logits: torch.Tensor, labels: torch.Tensor):
        """Stats du gradient analytique ∇_ℓ L_CE, accumulables entre batches."""
    ...
    def _rho_from_eta_stats(model, probs_sum, label_counts, n_valid, W_O):
        """Calcule rho depuis stats accumulées. Sépare la projection W_O."""
        eta_mean = (probs_sum - label_counts) / max(n_valid, 1)
        ...
        eta_mean = eta_mean.float() @ lm_head_weight.float()
        g = torch.einsum('hdi,d->hi', W_O.float(), eta_mean.float())
        norms = g.norm(dim=1, keepdim=True).clamp(min=1e-8)
        g_normed = g / norms
        return g_normed @ g_normed.T
    ...
    def compute_gradient_coupling(
        ...
        """ρ_ij sur un seul batch. Préférer log_head_matrices (N batches)."""
...
# ---------------------------------------------------------------------------
# Hook pour capturer les head outputs avant o_proj  (Qwen2)
# ---------------------------------------------------------------------------


def _get_qwen_layer(model, design_layer: int):
    """Retourne le module Qwen2DecoderLayer à l'index `design_layer`.

    Gère à la fois le modèle PEFT-wrappé (model.base_model.model.model.layers[i])
    et le modèle nu (model.model.layers[i]).
    """
    base = model
    # PeftModel : .base_model.model est le modèle HF natif
    if hasattr(base, "base_model") and hasattr(base.base_model, "model"):
        base = base.base_model.model
    # Le modèle HF Qwen2ForCausalLM : .model.layers
    if hasattr(base, "model") and hasattr(base.model, "layers"):
        return base.model.layers[design_layer]
    if hasattr(base, "layers"):
        return base.layers[design_layer]
    raise RuntimeError("Impossible de localiser model.model.layers pour Qwen")


class HeadOutputCapture:
    """
    Forward pre-hook sur self_attn.o_proj pour capturer les sorties par tête
    AVANT la projection. Architecture Qwen2 : input shape (B, T, hidden_size)
    avec hidden_size = n_heads * d_head = 14 * 64 = 896.
    """
    ...
    def register(self, model, design_layer: int = 19):
        layer = _get_qwen_layer(model, design_layer)
        attn_module = layer.self_attn
        cfg = model.config if hasattr(model, "config") else attn_module.config
...
def get_output_projection_weights(model, design_layer: int = 19, detach: bool = True) -> torch.Tensor:
    """
    Extrait les poids W_O (= o_proj.weight) effectifs reshapés par tête.

    Architecture Qwen2 : model.(...).layers[l].self_attn.o_proj  (896, 896)

    Returns: W_O (H, d, d_h) = (14, 896, 64)
    """
    layer = _get_qwen_layer(model, design_layer)
    o_proj = layer.self_attn.o_proj

    W_O_full = o_proj.weight.detach() if detach else o_proj.weight

    # Avec LoRA : poids effectif = W_base + scaling · lora_B @ lora_A
    if hasattr(o_proj, "lora_A") and "default" in o_proj.lora_A:
        scaling = o_proj.scaling["default"]
        lora_A = o_proj.lora_A["default"].weight
        lora_B = o_proj.lora_B["default"].weight
        if detach:
            lora_A = lora_A.detach()
            lora_B = lora_B.detach()
        W_O_full = W_O_full + scaling * (lora_B @ lora_A)

    cfg = model.config if hasattr(model, "config") else layer.self_attn.config
    num_heads = cfg.num_attention_heads        # 14
    head_dim = cfg.hidden_size // num_heads    # 64
    d = W_O_full.shape[0]                       # 896

    # W_O.shape == (d_out, n_heads * d_head). Les colonnes sont organisées par
    # tête : col [h*d_h, (h+1)*d_h) correspond à la tête h.
    W_O_heads = W_O_full.view(d, num_heads, head_dim)  # (896, 14, 64)
    W_O_heads = W_O_heads.permute(1, 0, 2)              # (14, 896, 64)

    return W_O_heads


# ---------------------------------------------------------------------------
# Visualisation et logging des matrices d'interaction
# ---------------------------------------------------------------------------


def _matrices_figure(omega: torch.Tensor, rho: torch.Tensor, G: torch.Tensor):
    """1×3 heatmaps : omega, rho, G."""
    mats = [
